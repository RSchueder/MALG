...
\end{flushright}
The physiological equations described in \cite{broch2012} do not mention any physical description of the seaweed fronds in space aside from their area. In fact, area (m$^{2}$) is the main state variable in the Broch model. It was found that the translation from an area based model to a DELWAQ mass based was non-trivial but possible. This is primarily because Broch uses a constant conversion factor between area and mass (called $ArDenMAL$ in MALG), and employs this factor in many of the model equations where mass rates concerning the structual mass are involved. Therefore, application of Broch's area based equations to DELWAQ's mass based system is possible given Broch's fixed mass to surface area frond ratio and a DELWAQ segment surface area. In most cases the equations involving $MALS$ in MALG are simply \textit{not} multiplied by $ArDenMAL$ as the units are already in mass and not area. The largest implications of the difference between Broch and MALG relate to photosynthesis, which is proportional to the frond area and not the biomass. For these calculations, the $MALS$ mass is simply multiplied by the segment surface area and divided by the $ArDenMAL$ to obtain the frond area in the segment. For fluxes, a conversion is made to obtain the units of g m$^{-2}$ or g m$^{-3}$.  
...
\begin{itemize}
\item $FootDepth$ The depth below the water surface that the foot of the frond is attached. The frond begins growing from the segment that intersects this depth. The segment it resides in can change with a variable water level as it is re-assigned each time step.
...
\item $LinDenMal$ The linear density of the culture. This describes the amount of mass it takes a single frond in 1 m$^{2}$ of culture to grow 1 m. All fronds in a single segment change length at the same rate.
\item $N_{frond}$ The number of fronds within a m2.
...
\begin{equation}
	...
	ELSE
		LinDenFact = 0.0
	END
	LenMAL = min(LinDenFact \times F_{stretch} + \frac{MALS}{LinDenMAL \times N_{frond}}, abs(LmaxMAL))
\end{equation}
\begin{equation}
...
Note that $N_{frond}$ is said to be $\ge 1$ even though there are certainly situations where it is not the case over a large segment area. The reason that $N_{frond}$ must be specified is to act as a scaling factor on $LinDenMAL$ and $mrtMAL$ (see process FLMALS), since they are defined by default on the basis of a single frond growing 1 m in a 1 m$^{2}$ box. When there are more than 1 fronds in that box, then the biomass required to increase the length of the culture increases. When biomass densities are extremely low (perhaps due to large grid sizes compared to the size of the farm), then it is not meaningful to think of anything other than an equal distribution of fronds across each m$^{2}$ of the grid cell, because any frond will always experience itself. Additionally this is a central assumption of most biogeochemical models (well mixed within a single segment). In these situations a very low biomass density multiplied by $LinDenMAL$ will yield a very short plant. The exchange with the ambient water will be correct but the vertical discretization will not be. 

To correct for this, the parameter $LinDenCor$ can be specified as = 1 to make the frond skinnier in order to have its length deviate from what would otherwise be calculated based on the mass per m$^2$. Setting $LinDenCor$ = 1 will make the culture possess the length of a normal individual frond without changing the biomass. A value of $LinDenCor$ = 0 will assign a length in accordance with the biomass present in the cell and no correction will be applied (this is the default setting). The correction factor is recommended for situations where the number of fronds per m${2}$ would be \textless\textless 1. This intercept $LinDenFact \times F_{stretch}$ is determined by the initial biomass condition and is only calculated at the beginning of the simulation and does not change. To further adjust this length beyond correcting for biomass density, i.e. to incorporate the effect of ropes, you can adjust the factor $FrondStrech$. This will simply multiply the computed length by a constant to elongate the frond and spread the biomass and the fluxes over more segments.
...
This formulation is designed to allow small fronds to grow faster than bigger fronds, and as such the value in Broch describes the area above which a frond will stuggle to grow well. This has large implications for the DELWAQ model in contrast to the Broch model which is 'individual based' rather than 'culture based' and thus can integrate this formulation seamlessly. The frond area $LocArea$ in a DELWAQ segment does not represent a single frond, but the sum of all frond areas in a segment (a culture). This means that the default value for $MALS_{0}$ cannot be used in the case where the DELWAQ user expects more than one frond per segment. It could be possible to require that the user specify $LinDenMAL$ and $MALS_{0}$ based on their knowledge of how many fronds they expect there to be in a segment. However, it is thought that knowledge of the number of fronds per m$^{2}$ is more likely to be a determinable value. Thus, the following formulation is used in the code to reduce the segment-based mortality for cultures with more than one frond per m${^2}$.

The default value for $LinDenMAL$ is 120 g m$^{-3}$. This is derived from the relationship between mass gain and length increase seen in \cite{sjotun1993}. The default value for $MALS_0$ and 0.06 m$^{-1}$. This is taken from \cite{broch2012}, meaning that it is for a single frond. These formulations describe how as the number of fronds m$^{-2}$ increases, the area limit within the segment increases and the amount of carbon required to increase the length of the culture in the segment increases, and both are proportional to what is known about the behaviour of a single frond.
...
$LocArea$ & Area of culture per m$^{2}$ of segment (m$^{-2}$)
\end{tabular}

Here it can be seen that using the logic described for the implementation of the density limitation, the mortality rate is adjusted to account for the presence of more than one frond per m${^2}$. This correction is in place to account for the fact that the individual fronds will not erode faster simply because there is a higher packing density (N$_{frond}$ \textgreater 1). When the packing density is high, the culture erosion parameter is decreased to account for the higher biomass density. If this were not implemented, the rate of erosion per unit biomass in both segments shown in ~\ref{fig:ambiguoussituation} would be identical because the mass and area are identical. In reality this would not be the case, since the area of any given small frond is 1/4th of the area of the large frond and the smaller fronds will erode less.
...
The main difference comes from the fact that if you take the same biomass and spread it over a larger area, the size of each frond in a m2 (because there necessarily is a frond in a m2) is very small, and so its growth rate is quite high. What you don't know is really what the distribution of the biomass within the cell will be. You actually want to determine the growth based on the size of the frond. to know this you need to know more about the true contrained space the frond lies within
