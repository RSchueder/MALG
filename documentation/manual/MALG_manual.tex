...
\begin{equation}
\phi = \frac{10^{-6}exp(\epsilon\times LocArea)}{1+10^{-6}exp((\epsilon\times LocArea)-1)}
\end{equation}
...
The irradiance is integrated over the current segment using the following equation:

\begin{equation}
I = \frac{-I_{0}}{ExtVl \times Depth} e^{-ExtVl \times LocalDepth} - e^{-ExtVl \times ()LocalDepth - Depth)}
\end{equation}
...
Currently the algae do not attenuate light in the water column.
