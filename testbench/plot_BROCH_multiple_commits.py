...
runs = {'broch_b9dce3e'         : 'manual',
        'broch_a743345'         : 'length',
        'broch_83148f7'         : 'add flux',
        'broch_e2830a6' :'DoPO4'}

for rr in runs.keys():
    plt.close('all')
    file = r'd:\projects\IMPAQT\MALG\testbench\%s\broch_col.his' % rr
    ...
    ax2.legend()
    pylab.savefig((r'%s\Temp_NO3.png' % printDirectory) ,dpi = 300)
    ...
    ax1.xaxis.set_major_formatter(xfmt)
    plt.legend()
    pylab.savefig((r'%s\irradiance.png' % printDirectory) ,dpi = 300)
    ...
    for ii in range(0,2):
        ...
        if ii != 0:
            pylab.savefig((r'%s\frond_area.png' % printDirectory ) ,dpi = 300)
        ...
        plt.legend()
        if ii != 0:
            pylab.savefig((r'%s\production.png' % printDirectory ) ,dpi = 300)
        ...
        plt.ylim([0.18,0.40])
        if ii != 0:
            pylab.savefig((r'%s\carbon_storage.png' % printDirectory) ,dpi = 300)
        ...
        ax.xaxis.set_major_formatter(xfmt)
        if ii != 0:
            pylab.savefig((r'%s\nitrogen_storage.png' % printDirectory) ,dpi = 300)
    ...
    ax.set_xlabel('time')
    ax.xaxis.set_major_formatter(xfmt)
    pylab.savefig((r'%s\carbon_budget.png' % printDirectory) ,dpi = 300)
